...
class DatabaseHelper(private val context: Context) :
    ...
    override fun onCreate(db: SQLiteDatabase?) {
        ...
        db?.execSQL("INSERT INTO users (Type,Name,Password) VALUES('Manager','admin','admin')")
    ...
    fun insertSellingProcess(name: String, price: String, stock: String, productId: String): Long {
        val values = ContentValues().apply {
            put("ProductNumber", productId)
            put("Quantity", 1)
            put("PriceSell", price.toDouble())
            put("Amount", 1*price.toDouble())
        }
        val db = writableDatabase
        return db.insert("selling_process", null, values)
    }

    fun updateSellingProcess(productId: String, price: String){
        val db = writableDatabase
        val tableName = "selling_process"
        val whereClause = "ProductNumber = ?"
        val whereArgs = arrayOf(productId)
        val values = ContentValues().apply {
            put("Quantity", (getQuantity(productId)+1))
            put("Amount", (getQuantity(productId)+1)*price.toDouble())
    ...
    fun getQuantity(productId:String):Int{
        val db = readableDatabase
        var quantity = -1
        val query = "select * from selling_process WHERE ProductNumber = '$productId'"
        val cursor: Cursor = db.rawQuery(query, null)
    ...
    fun updateProductStock(productId: String) {
        val db = writableDatabase
        val tableName = "products"
        val whereClause = "ProductNumber = ?"
        val whereArgs = arrayOf(productId)
        val values = ContentValues().apply {
            put("Stock", (getProductStock(productId)-1))
        }
        db.update(tableName,values,whereClause,whereArgs)
        db.close()
    }
